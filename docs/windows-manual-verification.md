...
| Open Refresh | Interval, notification, retention, manual update check, and startup update settings work |  |  |
...
| Click Check for Updates Now in Refresh settings | Latest-release status is shown and recorded without downloading or installing |  |  |
...
| List config backups from CLI with `--list-config-backups --limit 10` | App-owned top-level backups are listed with path, size, and timestamps without reading backup contents or changing files |  |  |
| Validate latest config backup from CLI with `--validate-latest-config-backup` | Newest app-owned backup is validated without changing `config.json` or `secrets/` |  |  |
