...
- First-run setup has a checklist, action targets, provider-specific setup decisions, and safe inline Manual/Mock/local app-server source actions, but it is not yet a full guided wizard for API credentials or advanced provider-specific setup.
...
5. Dogfood the guided first-run setup checklist and decide whether it should become a deeper wizard.
   A new user can see provider-specific setup decisions, apply safe Manual/Mock/local app-server choices inline, and jump to Providers or Privacy & Data from the setup panel now. API credential setup still intentionally routes through Privacy & Data and Providers.
