...
| Packaging and release | 7/10 | Self-contained publish, zip packaging, checksums, readiness gates, artifacts, draft release workflow, and update-check foundation exist. No installer, signing, or package apply step yet. |
...
- There is no installer, MSIX, code signing, package apply step, or uninstall story.
...
| Public binaries are not trusted by Windows | High | Zip, checksum, draft release workflow, and update-check metadata path exist | Add signing or at least documented install warnings before public release. |
...
8. Build the update download/install path.
   The app can now check GitHub release metadata. Next it needs checksum-verified download, staged extraction, and a safe replace/restart flow.
