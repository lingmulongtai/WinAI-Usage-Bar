...
Latest published release: `v0.1.6`. Current release candidate: `v0.1.7`, collecting the post-`v0.1.6` crash-report preview, recovery dogfooding, Settings-page smoke target, same-install preflight metadata, Codex startup-output sanitization, and Widget topmost persistence fixes needed before the next update dogfood pass.
...
| Packaging and release | 9/10 | Self-contained publish, zip packaging, checksums, readiness gates, artifacts, release workflow, English release notes generated from the changelog, published setup assets, unsigned installer warning, update-check, verified download, install-script preparation, rollback-capable guarded script launch with post-install smoke validation, explicit latest-update install orchestration, setup installer artifact/release paths, throttled startup update policy, current-version update dogfooding, published disposable release-to-release update dogfooding, and same-install update dogfood checklist/report scaffolding exist. No signing yet. |
...
- `v0.1.6` is published as the latest release with English release notes plus zip, zip checksum, setup executable, and setup checksum assets, and the latest-release endpoint resolves it correctly. The `v0.1.7` candidate is prepared to make the latest-release endpoint newer than installed `v0.1.6` copies once tagged. Update checks now surface and persist the latest GitHub Release page, required zip package/checksum asset names, and setup installer asset visibility while keeping the zip package as the required self-update artifact.
...
- DPI changes, alternate taskbar edges, reboot/sign-out startup behavior, real notification delivery, installer behavior, and same-install release-to-release updates still need real Windows manual testing. Widget topmost persistence has a focused fix and unit coverage, but needs another real shell run after the `v0.1.7` candidate is built.
...
- The latest release line is still moving quickly; `v0.1.7` should be treated as a dogfood patch once published, not a polished public utility.
...
| Self-update can damage an install | High | SHA256 verification, app-owned update staging, guarded script launch, same-version relaunch suppression, rollback on copy or post-install smoke-test failure, retained validation log metadata, unsafe zip-entry rejection, UTF-8 BOM generated scripts for non-ASCII paths, current-version update dogfooding, published disposable release-to-release update dogfooding with normal app-data unchanged assertions, and same-install dogfood checklist/report scaffolding | Dogfood same-install release-to-release update flows repeatedly before recommending automatic install. |
...
1. Dogfood release-to-release update flows.
   `v0.1.1` seeing `v0.1.2`, `v0.1.2` seeing `v0.1.3` with a legacy app-data guard, current-updater simulated `0.1.2 -> v0.1.3`, current-updater simulated `0.1.3 -> v0.1.4`, and published `v0.1.3 -> v0.1.4` download/prepare/apply against a disposable install have passed. The startup-policy helper is now guarded to source releases that actually expose `--run-startup-update-check`, and same-install checklist/report scaffolding exists. Next, repeat real same-install release-to-release update checks before trusting automatic install by default.

2. Continue the manual Windows verification checklist.
   A 2026-07-12 local run covered hidden-overflow tray access, context menu commands, compact panel, Settings, Widget display, and Exit, and the Widget topmost persistence bug from that run is fixed in the `v0.1.7` candidate. Next, rerun that path and then cover notifications, startup registration, DPI, alternate taskbar positions, and repeated multi-monitor placement.
