...
- Guided first-run checklist state, Provider Details, config backup export, backup validation, confirmed CLI restore by path, confirmed latest-backup CLI restore, latest-backup in-app restore, and confirmed reset-to-default recovery are implemented.
...
- Config backup and reset recovery now exist with basic decision guidance and both path-based and latest-backup CLI restore entries, but they still need repeated dogfooding before they can be treated as comfort features.
...
6. Dogfood config backup, restore, and reset.
   CLI restore by path, latest-backup CLI restore, latest-backup in-app restore, and reset-to-default recovery exist now, but they need repeated real-use recovery checks before they become comfort features.
