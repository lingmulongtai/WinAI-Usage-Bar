...
- Provider cards and Provider Details now include stale/future snapshot timestamp warnings. Provider cards redact displayed status/error text, and Provider Details includes non-secret repair guidance for warning, auth-required, unsupported, error, and unknown provider states, including more specific Claude/Claude Code CLI repair guidance.
