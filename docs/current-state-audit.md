...
Latest published release: `v0.1.6`. It adds post-`v0.1.5` Copilot metrics failure-state hardening, a CLI-only `--refresh-once` composition fix, stronger provider dogfooding readiness gates, and the recent native Settings activation work from the `v0.1.5` line.
...
- DPI changes, alternate taskbar edges, reboot/sign-out startup behavior, real notification delivery, installer behavior, and same-install release-to-release updates still need real Windows manual testing. Widget topmost persistence has a confirmed failure tracked in #213.
...
- The latest release line is still moving quickly; v0.1.6 should be treated as a dogfood patch, not a polished public utility.
...
| Tray/window behavior differs across Windows setups | Medium | Placement service, compact panel edge-case tests, single-instance guard tests, manual checklist, report scaffold, and one local shell verification run covering hidden-overflow tray access, context menu commands, compact panel, Settings, Widget display, and Exit | Fix #213, then run and record the checklist across taskbar edges, DPI, multi-monitor, startup, and theme modes. |
...
2. Continue the manual Windows verification checklist.
   A 2026-07-12 local run covered hidden-overflow tray access, context menu commands, compact panel, Settings, Widget display, and Exit. Next, fix Widget topmost persistence (#213), then cover notifications, startup registration, DPI, alternate taskbar positions, and repeated multi-monitor placement.
