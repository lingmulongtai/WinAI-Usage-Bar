...
Latest published release: `v0.1.7`. It is a dogfood patch that includes the post-`v0.1.6` crash-report preview, recovery dogfooding, Settings-page smoke target, same-install preflight metadata, Codex startup-output sanitization, Widget topmost persistence fix, and first real same-install `v0.1.6 -> v0.1.7` update pass.
...
| Windows shell integration | 7/10 | Tray, windows, placement, startup registration, and notifications exist, with duplicate notification suppression, compact panel placement edge-case tests, a manual verification checklist, report scaffold, an opt-in packaged-app UI launch smoke helper, and a 2026-07-12 local shell run that verified hidden-overflow tray menu, compact panel, Settings, Widget display, and Exit. The Widget topmost persistence regression found in that run is fixed in `v0.1.7`. |
| Product usability | 6/10 | The app now has guided first-run checklist state, provider-specific setup decisions with API setup steps and safe inline source actions, provider details, backup export/restore, recovery checks, and an isolated backup/restore/reset dogfood pass, but it is still mostly useful with Mock/Manual data today. |
| Packaging and release | 9/10 | Self-contained publish, zip packaging, checksums, readiness gates, artifacts, release workflow, English release notes generated from the changelog, published setup assets, unsigned installer warning, update-check, verified download, install-script preparation, rollback-capable guarded script launch with post-install smoke validation, explicit latest-update install orchestration, setup installer artifact/release paths, throttled startup update policy, current-version update dogfooding, published disposable release-to-release update dogfooding, same-install update dogfood checklist/report scaffolding, and one real same-install `v0.1.6 -> v0.1.7` update pass exist. No signing yet. |
| Test confidence | 8/10 | Core, infrastructure, view model, CLI, storage, parser, refresh, app-composition smoke, headless UI composition smoke, packaging smoke paths, UI launch smoke parser/script syntax, Copilot CLI refresh-once dogfood, and local packaged Settings activation dogfooding are covered without external CLIs. Visual UI runtime coverage remains limited. |
| Observability and support | 8/10 | Diagnostics summary, provider repair guidance, recovery guidance, redacted export, health report, isolated app-data dogfooding, provider dogfooding notes, release dogfooding notes, recovery dogfooding notes, validation log metadata, logs, recent crash report metadata UI, local redacted crash reports, and a bounded redacted crash message preview are solid for an MVP. No remote crash reporting yet. |
...
- Diagnostics exports redact common secret shapes, account identifiers, scope/reference fields, local user profile paths, and CLI override paths, use bounded redaction context for truncated large files, include a non-secret manifest summary, omit local app-data and secret-store root paths, and exclude `secrets/`.
...
- Local UI launch smoke coverage can now start the packaged app, activate either a minimal WinUI smoke window or the real Settings window briefly with isolated app data, verify the process stays alive, and wait for a clean exit. The Settings target found and drove out native WinUI failures from `NavigationView`, `InfoBar`, and `ProgressBar` usage in the current desktop environment. CI verifies the helper's syntax-only path so headless runners do not need interactive WinUI activation.
...
- Release dogfooding now includes a disposable prepared-update apply script that refuses to apply outside its work directory, falls back safely when redirected GUI output mangles non-ASCII result paths, verifies validation log metadata for current generated scripts, a published-release discovery helper with legacy app-data guards, a published startup-policy helper guarded to source releases `v0.1.5` or newer, a current-updater full-flow helper that can simulate an older current version, a real published `v0.1.3 -> v0.1.4` disposable update run, a published disposable `v0.1.6 -> v0.1.7` update run with normal app-data unchanged assertions, a current-updater simulated `0.1.6 -> v0.1.7` apply run, and a real same-install `v0.1.6 -> v0.1.7` update pass that reconciled as `Succeeded` with post-install validation `Passed`.
...
- Guided first-run checklist state, provider-specific first-run setup decisions with API setup steps and inline Manual/Mock/local app-server actions, Provider Details, config backup export, CLI backup inventory, backup validation, latest-backup CLI validation, confirmed CLI restore by path, confirmed latest-backup CLI restore, latest-backup in-app restore, confirmed CLI reset-to-default recovery, confirmed in-app reset-to-default recovery, an isolated CLI recovery dogfood pass, and non-ASCII backup CLI output dogfooding are implemented.
...
- Codex app-server parsing now handles common quota alias shapes, token-counter aliases, credit/cost/token summary aliases, safe ratio/fraction aliases, coherent nested usage/rate-limit window objects or arrays, top-level usage window arrays, absolute and relative reset timestamp shapes, and generic reset-after aliases such as `resetAfter` and `retry_after` in addition to basic usage and rate-limit percentages, and the client can keep partial account/rate-limit/usage data when one optional method is unavailable. Optional app-server diagnostics now include method-name-only success, failure, or timeout status without logging response bodies. JSON-RPC response matching now uses only top-level envelope ids so notifications with nested ids are ignored, and initialize metadata reports the current app informational version. Codex snapshots also preserve whether the user selected `Cli` or `LocalAppServer` so diagnostics and provider cards do not misreport the source, shared Codex/ChatGPT app-server snapshots use provider-specific usage window labels, and the most constrained recognized usage/rate-limit window is promoted to the primary display slot.
- Headless `--refresh-once` can exercise the real enabled-provider refresh pipeline through a CLI-only composition and print safe snapshot summaries, secondary usage/rate-limit window details when present, plus non-secret repair guidance without opening WinUI windows, composing tray/window/update services, or sending notifications. It supports one-shot provider/source overrides for dogfooding paths such as Codex LocalAppServer and GitHub Copilot OfficialApi missing-scope handling, and the Copilot missing-scope path has passed a process-level Release exe run that returned `AuthRequired`, wrote snapshots/history, and left no temp files behind.
...
- Privacy & Data and `--health-report` now include non-secret storage pressure guidance for history, backups, diagnostics exports, crash reports, and diagnostics logs, plus recovery guidance for backup, restore, reset, diagnostics export choices, and persisted startup update status. Isolated backup/list/validate/latest-restore/reset dogfooding passed without touching a sentinel under `secrets/`, and backup CLI output now provides ASCII-safe file metadata plus UTF-8 redirected output for non-ASCII app-data paths.
- Privacy & Data now lists recent crash report metadata, including timestamp, source, exception type, app version, size, path, and parse status, and can show an explicit bounded redacted message preview without showing stack trace or context contents.
...
- Windows shell dogfooding now has a concrete manual verification checklist, a timestamped local report script, and a 2026-07-12 local report at `artifacts/verification/windows-verification-20260712-231704-7301a3c.md`. The run verified tray hidden-overflow access, context menu commands, compact panel launch, Settings launch, Widget launch, and Exit, filed #213 for Widget topmost persistence, and the resulting settings-overwrite fix is now in `v0.1.7`.
...
- GitHub Copilot support targets organization or enterprise metrics with safer missing-scope, missing-PAT, permission-denied, and CLI refresh-once failure states; personal usage still belongs in Manual mode.
- The UI is functional but still not visually or ergonomically proven with extended daily use.
- DPI changes, alternate taskbar edges, reboot/sign-out startup behavior, real notification delivery, installer behavior, and repeated same-install release-to-release updates still need real Windows manual testing. Widget topmost persistence has a focused fix and unit coverage, but needs another real shell run after the `v0.1.7` build.
- There is no MSIX, code signing, or installer trust story yet beyond a documented unsigned installer warning and checksum verification guidance. Startup auto-update policy exists, disposable prepared-apply dogfooding has passed, the current updater can simulate older-version full update flows, published disposable update runs have passed, and one real same-install `v0.1.6 -> v0.1.7` update install has passed, but same-install and startup-policy install flows still need careful repetition before automatic install should be treated as safe-by-default.
- The latest release line is still moving quickly; `v0.1.7` should be treated as a dogfood patch, not a polished public utility.
- First-run setup has a checklist, action targets, provider-specific setup decisions, API setup step lists, and safe inline Manual/Mock/local app-server source actions, but it is not yet a full guided wizard with inline credential entry or advanced provider-specific setup.
...
- Local CLI discovery can still be messy on Windows. Provider Details has Codex WindowsApps and Claude CLI placeholder repair guidance, but deeper provider-specific repair checks are still needed for future CLI-backed providers.
...
| Tray/window behavior differs across Windows setups | Medium | Placement service, compact panel edge-case tests, single-instance guard tests, manual checklist, report scaffold, one local shell verification run covering hidden-overflow tray access, context menu commands, compact panel, Settings, Widget display, and Exit, and a focused fix for the Widget topmost persistence failure found in that run | Run and record the checklist across taskbar edges, DPI, multi-monitor, startup, and theme modes. |
...
| Self-update can damage an install | High | SHA256 verification, app-owned update staging, guarded script launch, same-version relaunch suppression, rollback on copy or post-install smoke-test failure, retained validation log metadata, unsafe zip-entry rejection, UTF-8 BOM generated scripts for non-ASCII paths, current-version update dogfooding, published disposable release-to-release update dogfooding with normal app-data unchanged assertions, same-install dogfood checklist/report scaffolding, and one real same-install `v0.1.6 -> v0.1.7` pass | Dogfood same-install and startup-policy install flows repeatedly before recommending automatic install. |
| Local data files grow too much | Medium | History retention by days and bytes; Privacy & Data storage pressure guidance includes history, backups, diagnostics exports, and diagnostics logs; backups and diagnostics exports can be pruned from UI or CLI while keeping newest matched files; recovery dogfooding covers backup count growth in isolated app data | Dogfood pressure thresholds and add richer compaction or per-folder controls if needed. |
| Config corruption causes user confusion | Medium | Corrupt config backup, default migration, unique temp files for config and backup saves, collision-resistant backup/export names, config export, CLI backup inventory, explicit and latest-backup validation, confirmed CLI restore by path, confirmed latest-backup CLI restore, latest-backup in-app restore, confirmed CLI reset-to-default recovery, confirmed in-app reset-to-default recovery, isolated CLI backup/list/validate/latest-restore/reset dogfooding, and recovery guidance | Repeat recovery dogfooding and tighten output/guidance based on real failures. |
| CLI availability is ambiguous on Windows | Medium | Safe health report checks command discovery, selected launch targets, repair hints, and short startup; command launch prefers resolved `.exe`/shim paths; Codex provider classifies startup failures; Provider Details gives generic, Codex-specific, and Claude-specific CLI placeholder repair guidance | Extend provider-specific repair checks to every future CLI-backed provider. |
...
1. Dogfood release-to-release update flows.
   `v0.1.1` seeing `v0.1.2`, `v0.1.2` seeing `v0.1.3` with a legacy app-data guard, current-updater simulated `0.1.2 -> v0.1.3`, current-updater simulated `0.1.3 -> v0.1.4`, published `v0.1.3 -> v0.1.4` download/prepare/apply against a disposable install, published disposable `v0.1.6 -> v0.1.7`, current-updater simulated `0.1.6 -> v0.1.7`, and real same-install `v0.1.6 -> v0.1.7` have passed. The startup-policy helper is now guarded to source releases that actually expose `--run-startup-update-check`. Next, repeat real same-install and startup-policy install checks before trusting automatic install by default.

2. Continue the manual Windows verification checklist.
   A 2026-07-12 local run covered hidden-overflow tray access, context menu commands, compact panel, Settings, Widget display, and Exit, and the Widget topmost persistence bug from that run is fixed in `v0.1.7`. Next, rerun that path and then cover notifications, startup registration, DPI, alternate taskbar positions, and repeated multi-monitor placement.
...
5. Dogfood the guided first-run setup checklist and decide whether it should become a deeper wizard.
   A new user can see provider-specific setup decisions, follow API setup steps, apply safe Manual/Mock/local app-server choices inline, and jump to Providers or Privacy & Data from the setup panel now. API credential setup still intentionally routes through Privacy & Data and Providers without inline secret entry.
...
8. Dogfood the opt-in UI launch smoke check.
   The helper can now open a minimal packaged WinUI smoke window or the real Settings window briefly and exit cleanly on a local Windows desktop; it still needs repeated runs across real desktop states and more Settings page navigation coverage.
...
10. Keep release readiness gates strict as distribution matures.
   Current gates cover metadata, audit date, smoke test, package, installer, checksum, the unsigned installer notice, same-install update dogfood guidance, and optional manual verification report evidence.
