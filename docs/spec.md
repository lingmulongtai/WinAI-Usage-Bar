...
The Refresh settings page must display the last non-secret update status in a readable summary. When recorded, the summary includes the last checked timestamp, startup update interval, status, latest version, current version, last launched install version, staged package path, prepared install script path, and message. User-controlled strings and paths must be redacted before display. Missing package/script/current-version fields should be omitted instead of shown as empty values.
