...
- Closing an already-open widget no longer overwrites a newly saved Always on top preference with the old window presenter state.
...
- Codex app-server reset parsing now recognizes generic relative aliases such as `resetAfter`, `retryAfter`, and `retry_after`.
...
- Provider Details now uses display-oriented redaction so sensitive key labels are collapsed in visible status and error text.
