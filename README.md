...
- Codex / ChatGPT: safe best-effort `codex app-server` JSON-RPC client and parser are implemented; provider settings can override the CLI command path; Windows command resolution prefers launchable `.exe`, `.cmd`, or `.bat` paths returned by `where.exe`; optional account/rate-limit/usage method failures or timeouts can return partial data, Codex snapshots preserve the selected `Cli` or `LocalAppServer` source kind, usage parsing recognizes top-level and nested usage/rate-limit windows, top-level usage arrays, and plural window containers with common quota aliases such as consumed/total/remaining plus percent variants, reset timestamps support ISO strings, Unix seconds, Unix milliseconds, relative reset seconds, and common reset-after aliases, and missing CLI/startup/auth failures return visible provider errors.
