...
- CLI `--prepare-update-install` can generate a PowerShell script that applies a staged update after the app exits, runs the updated app's `--smoke-test`, rolls back on validation failure, and writes `install-result.json`.
