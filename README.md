...
- Diagnostics can be exported from Privacy & Data; exports redact common secret shapes, account identifiers, scope/reference fields, and CLI override paths, use bounded redaction context when truncating large files, omit local app-data and secret-store root paths, never include files under `secrets/`, and avoid overwriting same-second exports by adding a numeric suffix when needed.
