...
- Refresh settings include interval, notification enablement, history retention limits, a manual update check button, a confirmation-gated latest-update install action, startup update checks, and optional verified auto-download/auto-install launch policy.
...
- CLI `--set-provider-cli-override` can save a non-secret command override for CLI/local app-server providers without echoing the value.
